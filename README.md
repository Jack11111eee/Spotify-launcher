...
**但它治不了「歌变灰」。** 实测过灰屏现场：Spotify 一直连着代理（`sp_conn` 7~15，从未断开）、代理探测也通、mihomo 日志里没有任何错误 —— 界面却灰了，而且重启 Spotify 也修不好。那个故障不在这一层，**根因至今没定位**，详见下面的诊断记录（那里也写明了哪些结论已经作废）。
...
当天最后**恢复正常播放**了，但**没能归因到某一个动作** —— 期间客户端状态被反复改动过（清 `primary.ldb`、整份移走 `PersistentCache` 导致重新登录、多次重启），无法判断是哪一步起的作用。**所以这次没有产出任何可复现的修法。**

当天确认下来的事实：

- 坏的时候，灰掉的曲目**确实不能播**，客户端对它们**连播放请求都不发**。
- 清掉 `primary.ldb` **只改变渲染**（`▶ MV ·` 前缀消失），**不恢复播放**。「可用性判定存在这个文件里」已被证伪。
- **渲染和可播性是解耦的**：当天最后，歌单里那几行**仍然显示 `▶ MV ·`，但同一首 `楓` 正常播放**（位置持续推进）。所以「看起来灰」不等于「不能播」。
...
**一条线索没验成**：客户端在 `extended-metadata` 请求体里上报的 `country` 是 **NG**。把它改写成 `TW` 后，某一轮里 `楓` 从「stopped」变成「playing」；但**关掉改写后 `楓` 也能播**，所以**改写并非必要条件**。这条线索既没被证实、也没被彻底排除，目前悬置。

**根因仍未定位。**

**注意：整份移走 `PersistentCache` 会把客户端登出** —— 登录态确实存在那里面（`prefs` 里的 `autologin.*` 不足以恢复）。要试必须先备份，且做好重新登录的准备。

**下面保留 2026-09-18 / 09-19 的原始记录。其中标了「已作废」的段落是当时的判断，事后被证伪，留作过程参考 —— 不要照着做。**
...
> 2026-09-19 复核：清掉 `primary.ldb` 只改变了**渲染**，并没有恢复播放；而且最后渲染仍显示 `▶ MV ·` 时曲目已经能正常播放。所以「可用性判定缓存在 `primary.ldb`、退回 MV 就是『灰』的来历」**不成立**。这段留作过程参考。
...
> 想靠改写请求来逼它刷新是走不通的：把 `revision=` 摘掉、或把上报的版本删掉，服务端都直接回 **`400`**（实测 21/21 和 22 次请求全部 400）。这两个参数是必填的。

#### 怎么定位到的

1. **先整份备份坏状态**（早先那次没来得及，坏状态被就地覆盖了）。这次在动手前把 `PersistentCache/` 整份存了下来。
2. 用 `spotify:playlist:<id>` 之类的方式把出问题的歌单调出来截图。
3. 试出「彻底退出 Spotify → 移走 `primary.ldb` → 重启」，`▶ MV ·` 前缀消失、标题恢复。
4. 反向验证：把坏的那份 `primary.ldb` 放回去、重启 —— `▶ MV ·` **立刻整片复现**（同一张截图里，第 1~8 行是 `▶ MV · 周杰倫`，第 9~10 行是正常的 `周杰倫`）。再换回干净的那份，恢复正常。

第 4 步只证明了**「`▶ MV ·` 这个渲染标记由 `primary.ldb` 驱动」**，当时把它当成了「坏状态就在这个文件里」的证据 —— **这是过度推断**：渲染被这个文件驱动，不等于可用性判定也在里面。后来单独测播放才发现两者是解耦的。
...
| 移走 `PersistentCache/`（**在 Spotify 运行时**） | 无效 | 进程开着 LevelDB，移动目录既不清内存状态也不清它已打开的文件，退出时还会把旧内容写回去 |
| 移走 `primary.ldb`（彻底退出后） | **只改渲染，不恢复播放** | 见上面「根因（已作废）」。当时误判为「有效」，后来单独测播放才发现没用 |
| 整份移走 `PersistentCache/`（彻底退出后） | **有害** | 会把客户端登出（登录态就在里面）。2026-09-19 实测踩到，已立即还原 |
| 摘掉 `/playlist/v2/.../diff` 的 `revision=` 参数 | 无效 | 服务端回 `400`，该参数必填 |
| 删掉 extended-metadata 请求体里上报的版本 | 无效 | 服务端回 `400`，该字段必填 |
| 把上报的版本号清零（保留字段） | 无效 | 服务端照回 `200` 完整元数据，界面仍灰 —— 刷新元数据**不等于**清掉可用性判定 |
| 把上报的 `country` 从 `NG` 改写成 `TW` | **不成立** | 某一轮里 `楓` 确实从 stopped 变 playing，但关掉改写后也能播，所以不是必要条件 |
...
**只有「能不能播」算数。** 所有看起来像的判据，这次都被证伪了：

- **标题的像素亮度**：小字号抗锯齿会让最大像素值偏低，量出来「已点赞的歌曲」这种确定可播放的列表，标题峰值也是同样的 **113**（大标题是 255）—— 跟灰掉的那张歌单数值一模一样，区分不出来。
- **artist 行前的 `▶ MV ·` 前缀**：一度以为这是结构性判据，比像素值可靠。**也被证伪** —— 当天最后，歌单里那几行仍显示 `▶ MV ·`，而同一首 `楓` 正常播放、进度条持续推进。它只反映渲染，不反映可播性。

所以判据只能是**行为性的**：点开播一下，看进度条走不走。

#### 诊断中走过的弯路

留在这里，免得以后重蹈：

1. **「根因是账号市场」——证据不支持。** 中途根据客户端 `customize` 响应里的 `country_code = "NG"`（尼日利亚区家庭组 Premium）判断「尼日利亚曲库小所以全灰」。但后续抓到 **46 次** `extended-metadata` 请求，`country` 全程都是 `NG`，**包括它正常播放音频的那几次**；`metadata/4/track` 在六个市场也都返回 200。所以 market 解释不了「灰」。改写成 `TW` 那一次观察到的变化，事后也没能复现为必要条件 —— 这条线索**既没被证实也没被彻底排除，目前悬置**。
2. **「移走 `PersistentCache` 会让 `/api/token` 持续 400」——错。** 实测那个 `400` 是 **DPoP 协议的正常握手**：服务端回 `{"error":"use_dpop_nonce"}` 并附上 nonce，客户端带上 nonce 重发随即 `200`。同一个请求连发两次、第一次 400 第二次 200，是设计如此，不是故障。（**但整份移走 `PersistentCache` 确实会把客户端登出** —— 那是登录态存在里面，与 `400` 无关，见上表。）

#### 下次再犯怎么办

**没有已知有效的修法。** 上面那版「清 `primary.ldb`」是错的（只改渲染，不改播放），已作废；这次最后虽然恢复了，但归因不了，不算办法。

已排除的：清 `~/Library/Caches/com.spotify.client/`、清 `primary.ldb`、换节点、强制服务端返回完整元数据（把 extended-metadata 上报的版本号清零 → 服务端照回 200，界面照样灰）。

下次该按这个顺序做：

1. **先备份，再动手。** 趁坏着，把 `PersistentCache/` 整份复制出去（**复制，不是移动** —— 移动会把你登出）。这次能事后回看，靠的就是那份存档。
2. **先用网页版做对照**：`open.spotify.com` 上打开同一张歌单。能播 → 客户端侧；不能播 → 账号/市场侧。这一步最便宜，且不改任何本地状态，应该第一个做。
3. **别急着清缓存。** 清 `primary.ldb` 只会让画面变好看、并不恢复播放，反而污染现场 —— 这次正是因为中途改了太多客户端状态，最后才归因不了。
4. 若确定是客户端侧，再逐个试 `public.ldb`、`Users/<id>/cached`，**一次只改一处**，每步都测播放。

判据只有一个：**能不能播**（见上面的「怎么判断灰没灰」）。

坏掉的那份已存档，放回去可以复现渲染层面的 `▶ MV ·`（但复现不了/修不好播放，见上）：

```
~/Library/Logs/spotify-grey-backup-20260919-181329/
  primary.ldb.BROKEN-confirmed     # 放回去能复现 `▶ MV ·` 渲染（播放仍不受影响）
...
这次没修好，但踩的坑值得记下来。按重要性排：

1. **别用外观代替行为做判据。** 我先后用过「标题像素亮度」和「`▶ MV ·` 前缀」判断灰没灰，**两个都被证伪** —— 前者区分不出可播列表，后者在能正常播放时依然显示。判据只能是「点了能不能播」。用替代指标时，要先拿一个**已知正确**的样本验证它能区分，否则量出来的数只是看着像。
2. **没有干净的 A/B，就不能宣布归因。** 这次中途反复改了客户端状态（清 `primary.ldb`、移走整份 `PersistentCache`、多次重启、改请求），最后恢复了却**无法归因**。想验证一个动作有没有用：**一次只改一处，改前改后各测一次播放**。做不到就别下结论 —— 我当时下了，紧接着就被实测证伪（歌还是灰的、还是不能播）。
3. **渲染和功能是两回事。** 清 `primary.ldb` 让画面恢复，我就当成修好了，其实播放压根没恢复。**界面变好看 ≠ 问题解决**，要在真正关心的那条链路上验证。
4. **拿名字去查 gid 之前先核对映射。** 从 LevelDB 按标题搜 gid，把周杰倫的《夜曲》匹配成了肖邦的，于是做了一组**完全无效的 A/B**（以为在测 `楓`，实际测的是 `髮如雪`）。后来改成用 metadata 接口返回的曲名反查才对上。凡是「按名字找 ID」的步骤，都要回读一次名字确认。
5. **别把用户的数据当草稿纸。** 整份移走 `PersistentCache` 直接把客户端登出了 —— 我当时以为登录态在 `prefs` 里，没先确认。**动用户本地状态前要先备份、并说清风险**；能只读验证的就别写。
6. **机制查清了，不等于根因找到了。** 这次把两层缓存的机制摸得很清楚（版本号 + 哈希、304、为什么它不发请求），但「本地那份数据里到底哪一项让它判定不可播」始终没查出来。**机制是通往根因的路，不是根因本身**，别把前者当成后者写进结论。
7. **走不通的路要写下来。** 改请求（摘 `revision=`、删版本号）一律 `400`，这类「试过、不行」的结论和成功的结论一样值钱 —— 否则下次还会再试一遍。
...
**一个容易被漏掉的细节**：默认抓包看到的是一片 `304 Not Modified`，看上去「一切正常」。要在插件里把 304 和请求体里的版本号打出来，才看得见「客户端一直在拿 304、一直在用本地那份数据」。这解释了为什么它对灰掉的歌**连请求都不发** —— 但注意，它**只解释了机制，不等于找到了根因**：本地那份数据里到底是哪一项让客户端认定「不可播」，至今没查出来。
...
> 别指望靠改请求来修：`revision=` 和上报的版本号都是必填，摘掉就 `400`（见上面的「两层缓存」）。抓包在这里的用途是**看清机制**，不是修 —— 而且这次看清了机制也没能定位根因。
